...
export default function LibraryPage() {
  ...
  const primaryTitle = articles[0]?.open_title ?? articles[0]?.title ?? "مراجعة كتاب البدع الحولية للشيخ عبد الله بن عبد العزيز التويجري ( فصل شهر رجب ) ";
  const handleShare = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const wanted = arr.filter((a) => (a.className ?? "").trim() === "arc-library");
        setArticles(wanted);
          ...
          <div className={styles.paper}>
            <div className={styles.paperInner}>
              <header className={styles.paperHeader}>
                <div className={styles.paperBadge}>اسم الكاتب : {(articles[0]?.author_name ?? '').trim() || 'عصارة الكتب'}</div>
                <h2 className={styles.paperTitle2}>{articles[0]?.open_title ?? articles[0]?.title ?? primaryTitle}</h2>
              </header>
