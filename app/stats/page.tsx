...
export default function StatsPage() {
  ...
  const primaryTitle = articles[0]?.open_title ?? articles[0]?.title ?? "إحصائيات وتحليلات";
  const handleShare = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const wanted = arr.filter((a) => (a.className ?? "").trim() === "arc-stats");
        setArticles(wanted);
          ...
          <div className={styles.paper}>
            <div className={styles.paperInner}>
              <header className={styles.paperHeader}>
                <div className={styles.paperBadge}>اسم الكاتب : {(articles[0]?.author_name ?? '').trim() || 'إحصائيات وتحليلات'}</div>
