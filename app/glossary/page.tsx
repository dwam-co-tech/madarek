...
export default function GlossaryPage() {
  ...
  const primaryTitle = articles[0]?.open_title ?? articles[0]?.title ?? "قاموس المصطلحات";
  const handleShare = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const wanted = arr.filter((a) => (a.className ?? "").trim() === "arc-glossary");
        setArticles(wanted);
          ...
          <div className={styles.paper}>
            <div className={styles.paperInner}>
              <header className={styles.paperHeader}>
                <div className={styles.paperBadge}>اسم الكاتب : {(articles[0]?.author_name ?? '').trim() || 'قاموس المصطلحات'}</div>
