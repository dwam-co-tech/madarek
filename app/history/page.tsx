...
export default function HistoryPage() {
  ...
  const primaryTitle = articles[0]?.open_title ?? articles[0]?.title ?? "بين “الوساطة” و“التهمة”: الشارف الغرياني في حرب برقة وموقفه من عمر المختار";
  const handleShare = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const wanted = arr.filter((a) => (a.className ?? "").trim() === "arc-history");
        setArticles(wanted);
          ...
          <div className={styles.paper}>
            <div className={styles.paperInner}>
              <header className={styles.paperHeader}>
                <div className={styles.paperBadge}>اسم الكاتب : {(articles[0]?.author_name ?? '').trim() || 'محطات تاريخية'}</div>
